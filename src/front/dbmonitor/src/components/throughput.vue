...
<div>
  <el-form :model='form' class='demo-ruleForm'   label-position='top'>
    <el-row :gutter='20' class='address'>
      ...
      <el-col :span='2'>
        <el-form-item>
          <el-button type="primary" @click="Querythroughput()">查询</el-button>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
  ...
  <el-form :model='form' :inline="true" class='demo-ruleForm'   label-position='left'>
    <el-row class='address'>
      ...
      <img :src="throughput[0].throughput" class="image" style="margin-left: 0%">
  </div>
</template>
<script>
import {throughput, hostnames, querythroughput, intervel} from '../mock/api'
export default {
  data: function () {
    return {
      hostnames: [],
  ...
  methods: {
    ...
    Querythroughput () {
      querythroughput({hostname_db: this.selectedOptions, times: this.times})
        .then((response) => {
          this.throughput = ([response.data])
          console.log(this.throughput)
        })
