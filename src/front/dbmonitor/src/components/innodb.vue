...
<div>
  <el-form :model='form' class='demo-ruleForm'   label-position='top'>
    <el-row :gutter='20' class='address'>
      ...
      <el-col :span='2'>
        <el-form-item>
          <el-button type="primary" @click="Queryinnodb()">查询</el-button>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
  ...
  <el-form :model='form' :inline="true" class='demo-ruleForm'   label-position='left'>
    <el-row class='address'>
      ...
      <img :src="innodb[0].innodb" class="image" style="margin-left: 0%">
  </div>
</template>
<script>
import {innodb, hostnames, queryinnodb, intervel} from '../mock/api'
export default {
  data: function () {
    return {
      hostnames: [],
  ...
  methods: {
    ...
    Queryinnodb () {
      queryinnodb({hostname_db: this.selectedOptions, times: this.times})
        .then((response) => {
          this.innodb = ([response.data])
          console.log(this.innodb)
        })
