...
<div>
  <el-form :model='form' class='demo-ruleForm'   label-position='top'>
    <el-row :gutter='20' class='address'>
      <el-col :span='1.5' style="margin-top: 5px;">
          <span class="demonstration">选择主机/数据库</span>
      </el-col>
      <el-col :span='4'>
        <el-form-item>
            <el-cascader
      ...
      <el-col :span='2'>
        <el-form-item>
          <el-button type="primary" @click="Querydbs()">查询</el-button>
        </el-form-item>
      </el-col>
    </el-row>
  </el-form>
  ...
  <el-form :model='form' :inline="true" class='demo-ruleForm'   label-position='left'>
    <el-row class='address'>
      ...
      <img :src="performance[0].performance" class="image" style="margin-left: -10%">
  </div>
</template>
<script>
import {performance, dbs, querydbs, intervel} from '../mock/api'
export default {
  data: function () {
    return {
      dbs: [],
  ...
  methods: {
    ...
    Querydbs () {
      querydbs({hostname_db: this.selectedOptions, times: this.times})
        .then((response) => {
  ...
  created () {
    performance()
      .then((response) => {
        ...
        this.db = response.data['db_name'][0]
      })
      .catch(function (error) {
        console.log(error)
      })
